  ...
  const getOptions = () => {
    ...
    if (title === "Event Manager") {
      return (<div className="space-y-3">
        <div><label className="text-xs text-muted-foreground">Action</label>
        <select value={action} onChange={e => setAction(e.target.value)} className="w-full bg-black/30 border border-border rounded-lg px-3 py-2 text-sm mt-1">
          <option>Start Event</option><option>Stop Event</option><option>View Active</option><option>Schedule</option>
        </select></div>
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button onClick={() => setActiveTool("server_ops")}
          className="px-4 py-2 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white rounded-lg text-xs font-bold hover:opacity-90 flex items-center gap-2">
          🛰️ Server Ops
        </button>
