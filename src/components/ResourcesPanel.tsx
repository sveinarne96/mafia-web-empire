...
function ResourceBar({ label, icon, value, max, color, textColor, regen, regenLabel }: {
  label: string; icon: string; value: number; max: number; color: string; textColor: string; regen?: number; regenLabel?: string;
}) {
    ...
    <div className="space-y-0.5">
      <div className="flex items-center justify-between text-xs">
        <span className="flex items-center gap-1">
          ...
          <span className={`font-black ${textColor}`}>{label}</span>
        </span>
  ...
  return (
    <div className="space-y-2 rounded-xl border border-amber-500/10 bg-black/20 p-2 shadow-[0_0_24px_rgba(245,158,11,0.04)]">
      ...
      <ResourceBar label="Energy" icon="⚡" value={resources.energy} max={resources.maxEnergy}
        color="bg-gradient-to-r from-yellow-500 to-amber-400" textColor="text-yellow-400" regen={resources.nextEnergyRegen} />
      <ResourceBar label="Stamina" icon="💪" value={resources.stamina} max={resources.maxStamina}
        color="bg-gradient-to-r from-green-500 to-emerald-400" textColor="text-green-400" regen={resources.nextStaminaRegen} />
      <ResourceBar label="Focus" icon="🧠" value={resources.focus} max={resources.maxFocus}
        color="bg-gradient-to-r from-blue-500 to-cyan-400" textColor="text-cyan-400" regen={resources.nextFocusRegen} />
      <ResourceBar label="Morale" icon="🔥" value={resources.morale} max={resources.maxMorale}
        color="bg-gradient-to-r from-orange-500 to-red-400" textColor="text-orange-400" />
      <ResourceBar label="Adrenaline" icon="💉" value={resources.adrenaline} max={resources.maxAdrenaline}
        color="bg-gradient-to-r from-red-600 to-pink-500" textColor="text-pink-400" />
      <ResourceBar label="Heat" icon="🔥" value={resources.heat} max={resources.maxHeat}
        color="bg-gradient-to-r from-red-700 to-red-500" textColor="text-red-400" />
    </div>
  );
}
