...
export function CriminalOperationsPage({ category }: { category: string }) {
  ...
  const run = async () => {
    if (busy || selectedLocked || cooldown > 0) return;
  ...
  return <div className="animate-fade-in space-y-4">
    ...
    <div className="grid gap-3 lg:grid-cols-[minmax(0,1.35fr)_minmax(260px,.65fr)]">
      <div className="grid gap-2 sm:grid-cols-2">
        {crimes.map((crime) => { const locked = level < crime.levelRequired; const active = crime.id === selected.id; return <button key={crime.id} onClick={() => !locked && setSelectedId(crime.id)} className={`rounded-xl border p-3 text-left transition-all ${locked ? "cursor-not-allowed border-slate-800/60 bg-slate-950/40 opacity-50" : active ? `border-${meta.tone}-400/50 bg-${meta.tone}-950/30 shadow-lg` : "border-slate-700/50 bg-slate-900/50 hover:border-white/20"}`}><div className="flex items-start gap-3"><span className="text-2xl">{locked ? "🔒" : meta.icon}</span><span className="min-w-0 flex-1"><span className="block text-sm font-bold text-white">{crime.name}</span><span className="mt-1 block text-[10px] leading-4 text-slate-400">{crime.description}</span></span></div><div className="mt-3 flex justify-between text-[10px]"><span className="text-green-400">${crime.reward.toLocaleString()}</span><span className="text-blue-400">+{crime.xp} XP</span><span className="text-red-400">{crime.risk}% risk</span></div></button>; })}
      </div>
      <section className="h-fit rounded-2xl border border-white/10 bg-slate-950/70 p-4 shadow-xl"><div className="mb-4 flex items-center justify-between"><div><div className="text-[10px] font-black uppercase tracking-widest text-slate-500">Selected operation</div><h2 className="mt-1 text-xl font-black text-white">{meta.icon} {selected.name}</h2></div><div className="rounded-full bg-red-500/15 px-2 py-1 text-[10px] font-bold text-red-300">{selected.risk}% risk</div></div><p className="text-xs leading-5 text-slate-400">{selected.description}</p><div className="mt-4 grid grid-cols-2 gap-2 text-center text-[10px]"><div className="rounded-lg bg-white/5 p-2"><div className="text-slate-500">Reward</div><div className="font-bold text-green-400">${selected.reward.toLocaleString()}</div></div><div className="rounded-lg bg-white/5 p-2"><div className="text-slate-500">XP</div><div className="font-bold text-blue-400">+{selected.xp}</div></div><div className="rounded-lg bg-white/5 p-2"><div className="text-slate-500">Unlock</div><div className="font-bold text-purple-300">Lv.{selected.levelRequired}</div></div><div className="rounded-lg bg-white/5 p-2"><div className="text-slate-500">Cooldown</div><div className="font-bold text-amber-300">{cooldown ? `${cooldown}s` : "Ready"}</div></div></div>{selectedLocked ? <div className="mt-4 flex items-center gap-2 rounded-xl border border-slate-700/50 bg-slate-900/70 p-3 text-xs text-slate-400"><Lock className="size-4" /> Reach level {selected.levelRequired} to unlock this operation.</div> : <button onClick={run} disabled={busy || cooldown > 0} className={`mt-4 w-full rounded-xl bg-gradient-to-r from-${meta.tone}-600 to-red-600 px-4 py-3 text-xs font-black text-white transition hover:opacity-90 disabled:opacity-50`}>{busy ? "Executing..." : cooldown ? `Cooldown · ${cooldown}s` : `Execute ${selected.name}`}</button>}{result && <div className={`mt-3 rounded-xl border p-3 text-xs ${result.success ? "border-green-500/30 bg-green-950/30 text-green-300" : "border-red-500/30 bg-red-950/30 text-red-300"}`}>{result.success ? `✅ Success · +$${result.money.toLocaleString()} · +${result.xp} XP` : `⚠️ Failed · ${result.message ?? "The operation went wrong."}`}</div>}<div className="mt-4 flex gap-2 text-[9px] leading-4 text-slate-600"><ShieldAlert className="size-3 shrink-0" /> Resource and success checks remain server-authoritative.</div></section>
    </div>
  </div>;
}
