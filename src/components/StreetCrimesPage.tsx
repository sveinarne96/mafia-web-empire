...
export function StreetCrimesPage() {
  ...
  return <div className="animate-fade-in space-y-5">
    <ActionHero eyebrow="Street operations network" title="The pavement pays." description="Choose your angle, watch your resources, and build a reputation one calculated move at a time." icon="🔪" accent="red" right={<div className="rounded-xl border border-red-400/25 bg-black/30 px-3 py-2 text-right"><div className="text-[9px] uppercase tracking-widest text-red-200/60">Current rank</div><div className="text-lg font-black text-red-300">LV.{level}</div></div>} />
    <div className="grid grid-cols-3 gap-2"><ActionStat icon="🎯" label="Unlocked" value={`${STREET_CRIMES.filter((crime) => level >= crime.level).length}/${STREET_CRIMES.length}`} tone="red" /><ActionStat icon="💰" label="Best payout" value={`$${Math.max(...STREET_CRIMES.map((crime) => crime.reward)).toLocaleString()}`} tone="green" /><ActionStat icon="⚡" label="Energy" value={`${energy}`} tone="amber" /></div>
    ...
    <div className="grid gap-3 lg:grid-cols-[minmax(0,1.35fr)_minmax(260px,.65fr)]">
      <div className="space-y-2">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {STREET_CRIMES.map((crime) => { const isLocked = level < crime.level; const isActive = crime.id === activeCrime.id; return <ActionCard key={crime.id} active={isActive} className={isLocked ? "cursor-not-allowed opacity-45" : ""}><button onClick={() => !isLocked && setSelected(crime.id)} disabled={isLocked} className="group w-full text-left"><div className="flex items-start gap-3"><span className="text-2xl transition-transform group-hover:scale-110">{isLocked ? "🔒" : crime.icon}</span><span className="min-w-0 flex-1"><span className="flex items-center gap-1 text-sm font-bold text-white">{crime.name}{isLocked && <Lock className="size-3 text-slate-500" />}</span><span className="mt-1 block text-[10px] leading-4 text-slate-400">{crime.description}</span></span></div><div className="mt-3 flex items-center justify-between text-[10px]"><span className="text-green-400">${crime.reward.toLocaleString()}</span><span className="text-blue-400">+{crime.xp} XP</span><span className="text-yellow-300">-{crime.energy} ⚡</span><span className="text-red-400">{crime.risk}% risk</span></div></button></ActionCard>; })}
        </div>
      </div>

      <section className="h-fit rounded-2xl border border-orange-500/25 bg-gradient-to-b from-orange-950/25 to-slate-950/70 p-4 shadow-xl shadow-black/20"><div className="mb-4 flex items-center justify-between"><div><div className="text-[10px] font-black uppercase tracking-widest text-orange-300">Operation brief</div>{resources && <div className="text-[10px] font-bold text-yellow-300">Energy now {energy}/{resources.maxEnergy}</div>}<h2 className="mt-1 text-xl font-black text-white">{activeCrime.icon} {activeCrime.name}</h2></div><div className="rounded-full bg-red-500/15 px-2 py-1 text-[10px] font-bold text-red-300">{activeCrime.risk}% risk</div></div><p className="mb-4 text-xs leading-5 text-slate-400">{activeCrime.description} Success is never guaranteed. A failed attempt can cost health, money, and freedom.</p><div className="mb-4 rounded-lg border border-yellow-500/20 bg-yellow-500/5 p-3 text-[10px] leading-4 text-yellow-100/80"><span className="font-bold text-yellow-300">⚡ Energy briefing:</span> starting this action uses {activeCrime.energy} energy. You recover 5 energy every minute. If energy reaches 0, actions unlock again as soon as you recover to 5 energy.</div><div className="grid grid-cols-2 gap-2 text-center text-[10px]"><div className="rounded-lg bg-black/25 p-2"><div className="text-slate-500">Reward</div><div className="font-bold text-green-400">${activeCrime.reward.toLocaleString()}</div></div><div className="rounded-lg bg-black/25 p-2"><div className="text-slate-500">Cooldown</div><div className="font-bold text-amber-300">{cooldown ? `${cooldown}s` : `${activeCrime.cooldown}s`}</div></div><div className="rounded-lg bg-black/25 p-2"><div className="text-slate-500">Energy per attempt</div><div className="font-bold text-yellow-300">-{activeCrime.energy} ⚡</div></div><div className="rounded-lg bg-black/25 p-2"><div className="text-slate-500">Unlock</div><div className="font-bold text-purple-300">Level {activeCrime.level}</div></div></div>{locked ? <div className="mt-4 flex items-center gap-2 rounded-xl border border-slate-700/50 bg-slate-900/70 p-3 text-xs text-slate-400"><Lock className="size-4" /> Reach level {activeCrime.level} to unlock this operation.</div> : <button onClick={() => execute(activeCrime)} disabled={busy || energyBlocked || cooldown > 0} className="mt-4 w-full rounded-xl bg-gradient-to-r from-red-600 to-orange-600 px-4 py-3 text-xs font-black text-white shadow-lg shadow-red-950/30 transition hover:from-red-500 hover:to-orange-500 disabled:cursor-not-allowed disabled:opacity-50">{energyBlocked ? "Recovering to 5 ⚡" : busy ? "Executing operation..." : cooldown ? `Cooldown active · ${cooldown}s` : `Execute ${activeCrime.name}`}</button>}{result && <div className={`mt-3 rounded-xl border p-3 text-xs ${result.success ? "border-green-500/30 bg-green-950/30 text-green-300" : "border-red-500/30 bg-red-950/30 text-red-300"}`}><div className="flex items-center gap-2 font-bold">{result.success ? "✅ Operation successful" : "⚠️ Operation failed"}</div>{result.success ? <div className="mt-1">+${result.money.toLocaleString()} · +{result.xp} XP</div> : <div className="mt-1">{result.text ?? "You were caught. Recover and try a different angle."}</div>}</div>}<div className="mt-4 flex items-start gap-2 text-[9px] leading-4 text-slate-600"><ShieldAlert className="mt-0.5 size-3 shrink-0" /> Every top-bar operation uses its listed energy cost. Energy regenerates by 5 every minute; below 5 energy, actions unlock again at 5 energy.</div>      </section>
    </div>
    <SafetyNote>Every operation has a visible risk profile. Spend energy deliberately, keep your heat manageable, and use cooldown windows to plan the next move.</SafetyNote>
  </div>;
}
