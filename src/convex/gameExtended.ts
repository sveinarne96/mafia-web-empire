...
export const commitLegendaryCrime = mutation({ args: { crimeId: v.string(), reward: v.optional(v.number()), xp: v.optional(v.number()), risk: v.optional(v.number()) }, handler: async (ctx, args) => { const p = await getPlayer(ctx); if (!p) throw new Error("Not authenticated"); const success = Math.random() < 0.3; const rw = success ? (args.reward ?? 100000) : 0; if (success) await ctx.db.patch(p._id, { money: (p.money ?? 0) + rw, experience: (p.experience ?? 0) + 250 }); return { success, reward: rw, message: success ? "Success!" : "Failed!" }; } });
...
export const buyEnergyDrink = mutation({ args: { drinkIndex: v.number() }, handler: async (ctx, args) => {
  ...
  await ctx.db.patch(p._id, {
    ...
    experience: ((p as any).experience ?? 0) + 5,
  } as any);
