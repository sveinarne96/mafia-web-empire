...
export const commitCrime = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Level-based success: 60% base + 0.5% per level, max 95%
    const levelBonus = Math.min(0.35, ((player.level ?? 1) * 0.005));
    const success = Math.random() < Math.min(0.95, 0.75 + levelBonus);
    switch (args.type) {
      ...
      case "rob_player": if (!args.targetId) throw new Error("Target required"); const target = await ctx.db.get(args.targetId); if (!target) throw new Error("Target not found"); moneyEarned = success ? Math.floor((target.money ?? 0) * 0.1) : 0; pointsEarned = success ? Math.floor(Math.random() * 151) + 50 : 0; damageTaken = success ? 0 : Math.floor(Math.random() * 30) + 10; break;
        ...
        const arrestThreshold = Math.max(0.15, 0.5 - (player.wantedLevel ?? 0) * 0.015); // FBI/Military actively hunting = higher arrest chance
    const arrested = !success && Math.random() > arrestThreshold;
    ...
    const boostMult = Date.now() < (player.xpBoostUntil ?? 0) ? 3 : 1; const energyMult = Date.now() < ((player as any).energyDrinkUntil ?? 0) ? 1.25 : 1; const xpGain = Math.floor((success ? 75 : 15) * boostMult * energyMult); const newXP = (player.experience ?? 0) + xpGain; const levelUpNow = newXP >= 2000;
...
export const fightPlayer = mutation({ args: { defenderId: v.id("users") }, handler: async (ctx, args) => {
    ...
    const fightXp = await addXpAndCheckLevel(ctx, attacker, 35); await ctx.db.patch(attacker._id, { life: fightXp.life ?? Math.max(0, (attacker.life ?? 100) - dDmg), totalFights: (attacker.totalFights ?? 0) + 1, totalKills: attackerWins ? (attacker.totalKills ?? 0) + 1 : (attacker.totalKills ?? 0), money: attackerWins ? (attacker.money ?? 0) + moneyStolen : Math.max(0, (attacker.money ?? 0) - moneyStolen), ...fightXp });
    if (!attackerWins) { 
...
export const dailyRaid = mutation({ args: { targetId: v.id("users") }, handler: async (ctx, args) => {
    ...
    await ctx.db.patch(player._id, { money: success ? (player.money ?? 0) + moneyStolen : (player.money ?? 0), dailyRaidUsed: (player.dailyRaidUsed ?? 0) + 1, ...(await addXpAndCheckLevel(ctx, player, success ? 50 : 15)) });
...
export const killPlayer = mutation({ args: { targetId: v.id("users") }, handler: async (ctx, args) => {
    ...
    if (success) { await ctx.db.patch(args.targetId, { life: 0, isDead: true }); await ctx.db.patch(player._id, { totalKills: (player.totalKills ?? 0) + 1, wantedLevel: Math.min(20, (player.wantedLevel ?? 0) + 3), money: (player.money ?? 0) + Math.floor((target.money ?? 0) * 0.05), ...(await addXpAndCheckLevel(ctx, player, 100)) }); }
    else { const killFailXp = await addXpAndCheckLevel(ctx, player, 20); await ctx.db.patch(player._id, { life: killFailXp.life ?? Math.max(0, (player.life ?? 100) - 30), wantedLevel: Math.min(20, (player.wantedLevel ?? 0) + 1), ...killFailXp }); }
    return { success };
  },
});
...
export const paroleHearing = mutation({ args: {}, handler: async (ctx) => { const player = await getCurrentUser(ctx); if (!player) throw new Error("Not authenticated"); const success = Math.random() > 0.25; if (success) await ctx.db.patch(player._id, { inPrison: false, prisonTime: 0, paroleEligible: false }); return { success }; } });
...
export const prisonFight = mutation({ args: { targetId: v.optional(v.id("users")) }, handler: async (ctx, args) => { const player = await getCurrentUser(ctx); if (!player) throw new Error("Not authenticated"); const success = Math.random() > 0.25; const reward = success ? Math.floor(Math.random() * 1000) + 200 : 0; if (success) await ctx.db.patch(player._id, { prisonCurrency: (player.prisonCurrency ?? 0) + reward }); return { success, reward }; } });
...
export const sparPlayer = mutation({ args: { targetId: v.id("users") }, handler: async (ctx, args) => { const player = await getCurrentUser(ctx); if (!player) throw new Error("Not authenticated"); const target = await ctx.db.get(args.targetId); if (!target) throw new Error("Target not found"); const won = (player.attack ?? 10) + Math.random() * 10 > (target.attack ?? 10) + Math.random() * 10; await ctx.db.patch(player._id, { ...(await addXpAndCheckLevel(ctx, player, won ? 25 : 8)), totalFights: (player.totalFights ?? 0) + 1 }); return { won }; } });
...
export const commitCategoryCrime = mutation({ args: { crimeId: v.string(), reward: v.number(), risk: v.number(), xp: v.number() }, handler: async (ctx, args) => { const player = await getCurrentUser(ctx); if (!player) throw new Error("Not authenticated"); if (player.inPrison) throw new Error("You are in prison!"); if (player.isDead) throw new Error("You are dead!");    // Level-based success rate: base 60% + 0.5% per level, max 95%
    ...
    const cashBoostOn = Date.now() < (player.cashBoostUntil ?? 0) ? 3 : 1; const moneyEarned = succeeded ? Math.floor((args.reward * cashBoostOn) + Math.random() * args.reward * 0.2) : -Math.floor(Math.random() * 500 + 100); const xpEarned = succeeded ? Math.floor(args.xp * (Date.now() < (player.xpBoostUntil ?? 0) ? 3 : 1) * (Date.now() < ((player as any).energyDrinkUntil ?? 0) ? 1.25 : 1)) : Math.floor(args.xp * 0.2); const lifeDamage = succeeded ? 0 : Math.floor(Math.random() * 20 + 5); const newLife = Math.max(0, (player.life ?? 100) - lifeDamage); const newXP = (player.experience ?? 0) + xpEarned; const levelUpNow = newXP >= 2000; const arrested = !succeeded && Math.random() > Math.max(0.15, 0.6 - (player.wantedLevel ?? 0) * 0.015); const pointsLevelMult = 1 + Math.min(2.0, ((player.level ?? 1) * 0.04)); const pointsEarned = succeeded ? Math.floor((Math.floor(Math.random() * 151) + 50) * pointsLevelMult) * (Date.now() < ((player as any).pointsBoostUntil ?? 0) ? 3 : 1) : 0; // Track completed crimes per category - no per-crime cooldown
    const crimeCategoryMap: Record<string, string[]> = {
      street: ["pickpocket","mugging","shoplifting","phone_snatch","bicycle_theft","purse_snatch","car_breakin","drug_rip","scrap_metal","garbage_scavenge","ticket_scalp","scam_call","panhandle","vandalism","egg_smuggle","bootlegging","fence_stolen","dumpster_dive","sneak_bus","atm_skim"],
      robbery: ["breaking_entering","smash_grab","convenience_store","gas_station","pawn_shop","pharmacy_robbery","atm_bombing","construction_theft","electronics_heist","credit_fraud","identity_theft","armed_robbery","jewelry_blitz","art_gallery","museum_heist","armored_car","bank_vault","casino_heist","yacht_theft","diamond_raid","train_robbery","cargo_hijack","drug_lab_raid","luxury_car_theft","power_plant","warehouse_raid","hospital_theft","charity_embezzle","church_theft","school_theft"],
      fraud: ["lottery_scam","tax_evasion","car_insurance","romance_scam","extortion","kidnapping","counterfeit","insurance_fraud","wire_fraud","money_laundering","ponzi","insider_trading","corp_espionage","ransomware"],
      burglary: ["suburban_casing","apartment_ransack","easy_house","average_house","vacation_home","luxury_villa","penthouse_raid","mansion"],
      drugs: ["corner_deal","drug_running","cook_operation","precursor_theft","manufacturing","distribution","lab_expansion","bulk_intercept","underground_lab","smuggling_ring","intl_supply","cartel_partner"],
      organized: ["protection","loan_sharking","racketeering","blackmail_op","extortion_ring","smuggling_syndicate","arms_traffic","human_traffic","laundering_empire","family_alliance","territory_takeover","underground_casino","prostitution_ring","political_corruption","judicial_bribery","police_corruption","tax_evasion_net","id_theft_ring","cyber_syndicate","border_bribery"],
      underground: ["counterfeiting","drug_trafficking","arson","underground_id","arms_dealing","underground_tax","underground_racket","underground_smuggle","witness_intim","jury_tamper","evidence_destroy","court_bribery","jailbreak","fight_ring","illegal_gambling","pirated_goods","organ_trade","cyber_attack","data_breach","deep_web","darknet","cryptojack","ransomware_deploy","zero_day"]
    };
    const categoryId = Object.keys(crimeCategoryMap).find(cat => crimeCategoryMap[cat].includes(args.crimeId));
    const prevCompleted: Record<string, string[]> = (player as any).crimeCompleted || {};
    const currentCompleted = [...new Set([...(prevCompleted[categoryId || ""] || []), args.crimeId])];
    const totalInCategory = categoryId ? (crimeCategoryMap[categoryId]?.length || 0) : 0;
    const allDone = currentCompleted.length >= totalInCategory && totalInCategory > 0;
    const newCompleted = { ...prevCompleted, [categoryId || ""]: currentCompleted };
    // Set 90s category cooldown ONLY when all crimes in category are done
    const cooldowns = allDone ? { ...(player.crimeCooldowns || {}), [categoryId || ""]: Date.now() + 90000 } : (player.crimeCooldowns || {}); await ctx.db.patch(player._id, { money: Math.max(0, (player.money ?? 0) + moneyEarned), life: newLife, totalCrimes: (player.totalCrimes ?? 0) + 1, experience: levelUpNow ? 0 : newXP, levelUpPending: levelUpNow ? true : (player.levelUpPending ?? false), inPrison: arrested, prisonTime: arrested ? 15000 : (player.prisonTime ?? 0), wantedLevel: arrested ? 0 : Math.min(20, (player.wantedLevel ?? 0) + (succeeded ? 1 : 0)), lastCrimeAt: Date.now(), crimeMomentum: Math.min(100, (player.crimeMomentum ?? 0) + 3), crimeCooldowns: cooldowns, crimeCompleted: allDone ? { ...newCompleted, [categoryId || '']: [] } : newCompleted, points: (player.points ?? 0) + pointsEarned } as any); await ctx.db.patch(player._id, { actionTimestamps: [,...((player as any).actionTimestamps ?? []).filter((t: number) => Date.now() - t < 3600000).slice(-999), Date.now()] } as any);
