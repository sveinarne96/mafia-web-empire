...
export const giveMoney = mutation({ args: { receiverId: v.id("users"), amount: v.number() }, handler: async (ctx, args) => { const player = await getCurrentUser(ctx); if (!player) throw new Error("Not authenticated"); if ((player.money ?? 0) < args.amount) throw new Error("Not enough money!"); const receiver = await ctx.db.get(args.receiverId); if (!receiver) throw new Error("Player not found"); await ctx.db.patch(player._id, { money: (player.money ?? 0) - args.amount }); await ctx.db.patch(args.receiverId, { money: (receiver.money ?? 0) + args.amount }); return { success: true }; } });export const commitCategoryCrime = mutation({ args: { crimeId: v.string(), reward: v.number(), risk: v.number(), xp: v.number() }, handler: async (ctx, args) => { const player = await getCurrentUser(ctx); if (!player) throw new Error("Not authenticated"); if (player.inPrison) throw new Error("You are in prison!"); if (player.isDead) throw new Error("You are dead!"); if (!Number.isFinite(args.reward) || !Number.isFinite(args.risk) || !Number.isFinite(args.xp)) throw new Error("Invalid operation parameters, please retry."); const levelBonus = 1; const succeeded = Math.random() < 0.95; let lootVehicle: string | null = null; let lootItem: string | null = null; const titleCase = (id: string) => id.split('_').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' '); if (succeeded && args.crimeId.startsWith('gta_')) { const spec = GTA_LOOT[args.crimeId]; const speed = spec ? spec.speed : Math.min(120, Math.max(30, Math.round(30 + (args.reward / 40000) * 90))); const storage = spec ? spec.storage : Math.min(60, Math.max(10, Math.round(10 + (args.reward / 40000) * 50))); const armored = spec ? !!spec.armored : args.risk >= 60; lootVehicle = spec ? spec.name : ('Stolen ' + titleCase(args.crimeId.slice(4))); await ctx.db.insert('vehicles', { userId: player._id, name: lootVehicle, type: 'stolen', speed, storage, armored, stolen: true, purchasePrice: spec ? spec.price : 0 }); } else if (succeeded && args.crimeId.startsWith('sh_')) { const spec = SH_LOOT[args.crimeId]; const rarity = spec ? spec.rarity : (args.reward >= 20000 ? 'legendary' : args.reward >= 5000 ? 'rare' : args.reward >= 1000 ? 'uncommon' : 'common'); lootItem = spec ? spec.name : (titleCase(args.crimeId.slice(3)) + ' Loot'); await ctx.db.insert('inventory', { userId: player._id, itemId: 'loot_' + args.crimeId, name: lootItem, type: 'loot', equipped: false, quantity: 1, attack: spec ? (spec.attack ?? 0) : Math.floor(Math.random() * 5) + 1, defense: spec ? (spec.defense ?? 0) : Math.floor(Math.random() * 5) + 1, rarity, price: spec ? spec.price : args.reward }); }
