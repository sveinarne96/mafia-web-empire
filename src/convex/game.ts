...
export const commitCrime = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const boostMult = Date.now() < (player.xpBoostUntil ?? 0) ? 3 : 1; const xpGain = Math.floor((success ? 15 : 3) * 11.0 * levelBonusXp * boostMult); const newXP = (player.experience ?? 0) + xpGain; const levelUpNow = newXP >= (player.level ?? 1) * 100;
