...
export const commitCrime = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const currentEnergy = typeof (player as any).energy === "number" && Number.isFinite((player as any).energy) ? (player as any).energy : 100;
    if (currentEnergy < 5) throw new Error("Recover at least 5 energy before acting again.");
