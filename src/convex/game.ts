...
export const commitCrime = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const success = Math.random() < Math.min(0.95, 0.6 + levelBonus);
    switch (args.type) {
...
export const fightPlayer = mutation({ args: { defenderId: v.id("users") }, handler: async (ctx, args) => {
    ...
    if (!attackerWins) await ctx.db.patch(args.defenderId, { money: (defender.money ?? 0) + moneyStolen, totalKills: (defender.totalKills ?? 0) + 1 });
...
export const dailyRaid = mutation({ args: { targetId: v.id("users") }, handler: async (ctx, args) => {
    ...
    const success = Math.random() > 0.4; const moneyStolen = success ? Math.floor((target.money ?? 0) * 0.08) : 0; const damage = Math.floor(Math.random() * 20) + 10;
...
export const levelUp = mutation({ args: { stat: v.optional(v.union(v.literal("attack"), v.literal("defense"), v.literal("maxLife"))) }, handler: async (ctx, args) => {
    ...
    const patch: Record<string, unknown> = { level: newLevel, levelUpPending: false, skillPoints: (player.skillPoints ?? 0) + 1, maxLife: (player.maxLife ?? 100) + 5, life: (player.maxLife ?? 100) + 5, attack: (player.attack ?? 10) + 1, defense: (player.defense ?? 10) + 1, highestLevel: Math.max(player.highestLevel ?? 0, newLevel) };
    if (args.stat === "maxLife") { patch.maxLife = (player.maxLife ?? 100) + 10; patch.life = (player.maxLife ?? 100) + 10; } else if (args.stat === "attack") { patch.attack = (player.attack ?? 10) + 2; } else if (args.stat === "defense") { patch.defense = (player.defense ?? 10) + 2; }
...
export const paroleHearing = mutation({ args: {}, handler: async (ctx) => { const player = await getCurrentUser(ctx); if (!player) throw new Error("Not authenticated"); const success = Math.random() > 0.5; if (success) await ctx.db.patch(player._id, { inPrison: false, prisonTime: 0, paroleEligible: false }); return { success }; } });
...
export const prisonFight = mutation({ args: { targetId: v.optional(v.id("users")) }, handler: async (ctx, args) => { const player = await getCurrentUser(ctx); if (!player) throw new Error("Not authenticated"); const success = Math.random() > 0.5; const reward = success ? Math.floor(Math.random() * 1000) + 200 : 0; if (success) await ctx.db.patch(player._id, { prisonCurrency: (player.prisonCurrency ?? 0) + reward }); return { success, reward }; } });
