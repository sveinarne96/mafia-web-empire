...
const HOURLY_CLICK_LIMIT = 200;
const HOUR_MS = 60 * 60 * 1000;

async function enforceHourlyClickLimit(ctx: { db: any }, player: any) {
  const now = Date.now();
  const recent = ((player as any).actionTimestamps ?? []).filter((t: unknown) => typeof t === "number" && now - t < HOUR_MS);
  if (recent.length >= HOURLY_CLICK_LIMIT) {
    const oldest = Math.min(...recent);
    const waitMinutes = Math.max(1, Math.ceil((HOUR_MS - (now - oldest)) / 60000));
    throw new Error(`Hourly action limit reached (200/200). Try again in about ${waitMinutes} minute${waitMinutes === 1 ? "" : "s"}.`);
  }
  const updated = [...recent, now].slice(-HOURLY_CLICK_LIMIT);
  await ctx.db.patch(player._id, { actionTimestamps: updated } as any);
  return updated;
}
...
export const commitCrime = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await enforceHourlyClickLimit(ctx, player);
