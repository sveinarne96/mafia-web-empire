...
export const commitCrime = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const xpGain = Math.floor((success ? 15 : 3) * 1.85 * levelBonusXp); const newXP = (player.experience ?? 0) + xpGain; const levelUpNow = newXP >= (player.level ?? 1) * 100 && !arrested;
