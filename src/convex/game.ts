...
export const commitCrime = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    switch (args.type) {
      case "car_theft": moneyEarned = success ? Math.floor(Math.random() * 5000) + 500 : 0; pointsEarned = success ? 5 : 0; damageTaken = success ? 0 : Math.floor(Math.random() * 20) + 5; break;
      case "burglarize": moneyEarned = success ? Math.floor(Math.random() * 3000) + 200 : 0; pointsEarned = success ? 3 : 0; damageTaken = success ? 0 : Math.floor(Math.random() * 15) + 5; break;
      case "rob_player": if (!args.targetId) throw new Error("Target required"); const target = await ctx.db.get(args.targetId); if (!target) throw new Error("Target not found"); moneyEarned = success ? Math.floor((target.money ?? 0) * 0.1) : 0; pointsEarned = success ? 8 : 0; damageTaken = success ? 0 : Math.floor(Math.random() * 30) + 10; break;
    }
