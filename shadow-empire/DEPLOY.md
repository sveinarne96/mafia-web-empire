...
Or create a `.env` file next to `package.json` — a template is included:
```bash
cp env.example .env
```
(Already points at your existing backend. Change `VITE_CONVEX_URL` inside if you deploy a fresh Convex project.)
